/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { ApiProperty } from "@nestjs/swagger";
import { QualityInspectionWhereInput } from "./QualityInspectionWhereInput";
import { ValidateNested, IsOptional } from "class-validator";
import { Type } from "class-transformer";

@InputType()
class QualityInspectionListRelationFilter {
  @ApiProperty({
    required: false,
    type: () => QualityInspectionWhereInput,
  })
  @ValidateNested()
  @Type(() => QualityInspectionWhereInput)
  @IsOptional()
  @Field(() => QualityInspectionWhereInput, {
    nullable: true,
  })
  every?: QualityInspectionWhereInput;

  @ApiProperty({
    required: false,
    type: () => QualityInspectionWhereInput,
  })
  @ValidateNested()
  @Type(() => QualityInspectionWhereInput)
  @IsOptional()
  @Field(() => QualityInspectionWhereInput, {
    nullable: true,
  })
  some?: QualityInspectionWhereInput;

  @ApiProperty({
    required: false,
    type: () => QualityInspectionWhereInput,
  })
  @ValidateNested()
  @Type(() => QualityInspectionWhereInput)
  @IsOptional()
  @Field(() => QualityInspectionWhereInput, {
    nullable: true,
  })
  none?: QualityInspectionWhereInput;
}
export { QualityInspectionListRelationFilter as QualityInspectionListRelationFilter };
